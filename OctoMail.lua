...
OctoMail.api = getfenv()
OctoMail.timer = 0
OctoMail.log = {}
OctoMail.orig = {}
OctoMail.hooks = {}
OctoMail.hook = setmetatable( {}, { __newindex = function( _, k, v ) m.hooks[ k ] = v end } )
OctoMail.debug_enabled = false

-- Drop an autocomplete name once it has gone unseen this long.
OctoMail.AUTOCOMPLETE_MAX_AGE = 60 * 60 * 24 * 30

-- Anything below this is a GetTime() value left by an older build -- a count of
-- seconds since that client launched, so at most a few million. A real time()
-- stamp is ~1.7 billion, so the two can never be confused.
OctoMail.LEGACY_TIMESTAMP_CUTOFF = 1000000000

-- Keep at most this many entries per log. The log is a SavedVariable written
-- on every mail sent or received and nothing ever trimmed it, so it grew for
-- the life of the character and was re-parsed in full at every login.
OctoMail.LOG_MAX_ENTRIES = 500

---@type Calendar
OctoMail.calendar = m.Calendar.new()

function OctoMail:init()
  self.debug( "OctoMail.init" )
  self.update_frame = m.api.CreateFrame( "Frame", "OctoMailFrame", m.api.MailFrame )
  ...
  m.api.OctoMail_AutoCompleteNames = {}
  ...
  self.tooltip_frame = m.api.CreateFrame( "GameTooltip", "OctoMailTooltipFrame", nil, "GameTooltipTemplate" )
  self.tooltip_frame:SetOwner( m.api.WorldFrame, "ANCHOR_NONE" );
end

---@param args string
function OctoMail.slash_command( args )
  if args == "" or args == "help" then
    m.api.DEFAULT_CHAT_FRAME:AddMessage( "|cffabd473OctoMail " .. L[ "Help" ] .. "|r" )
    m.api.DEFAULT_CHAT_FRAME:AddMessage( "|cffabd473/om log|r " .. L[ "Toggle logging on/off" ] )
    m.api.DEFAULT_CHAT_FRAME:AddMessage( "|cffabd473/om clear sent|r " .. L[ "Clear sent log" ] )
    m.api.DEFAULT_CHAT_FRAME:AddMessage( "|cffabd473/om clear received|r " .. L[ "Clear received log" ] )
    m.api.DEFAULT_CHAT_FRAME:AddMessage( "|cffabd473/om clear names|r " .. L[ "Clear saved recipient names from autocomplete" ] )
    return
  end

  if args == "log" then
    m.api.OctoMail_Log[ "Settings" ][ "Enabled" ] = not m.api.OctoMail_Log[ "Settings" ][ "Enabled" ]
    if m.api.OctoMail_Log[ "Settings" ][ "Enabled" ] then
  ...
  if string.find( args, "^clear" ) then
    ...
    elseif args == "clear names" then
      ...
      m.api.OctoMail_AutoCompleteNames[ key ] = {}
    end
  end
...
function OctoMail.CURSOR_UPDATE()
  m.cursorItem = nil
end

function OctoMail.get_cursor_item()
  return m.cursorItem
end

---@param item table
function OctoMail.set_cursor_item( item )
  m._cursorItem = item
end

function OctoMail.BAG_UPDATE()
  if m.api.MailFrame:IsVisible() then
    m.api.SendMailFrame_Update()
  end
end

function OctoMail.MAIL_SHOW()
  if m.api.OctoMail_Point then
    m.debug( "Set point" )
    m.api.MailFrame:SetPoint( m.api.OctoMail_Point.point, m.api.OctoMail_Point.x, m.api.OctoMail_Point.y )
  end
...
function OctoMail.MAIL_CLOSED()
  m.inbox_abort()
  m.sendmail_sending = false
  m.sendmail_clear()
end

function OctoMail.UI_ERROR_MESSAGE()
...
-- Carry data over from TurtleMail, which this is a fork of. The rename moved
-- the SavedVariables to OctoMail_*, so a straight swap would have looked like
-- a wiped mail log and a forgotten autocomplete list.
--
-- Only possible while TurtleMail is still installed and enabled: its own
-- SavedVariables file is what puts TurtleMail_* into the global namespace. Log
-- in once with both present, then remove TurtleMail. Runs once and is a no-op
-- after that, so nothing is clobbered if you later use both again.
function OctoMail.migrate_from_turtlemail()
  local moved = {}

  if m.api.TurtleMail_AutoCompleteNames and not m.api.OctoMail_Log[ "Settings" ].migrated then
    for key, names in m.api.TurtleMail_AutoCompleteNames do
      m.api.OctoMail_AutoCompleteNames[ key ] = m.api.OctoMail_AutoCompleteNames[ key ] or {}
      for name, seen in names do
        if not m.api.OctoMail_AutoCompleteNames[ key ][ name ] then
          m.api.OctoMail_AutoCompleteNames[ key ][ name ] = seen
        end
      end
    end
    table.insert( moved, "autocomplete names" )
  end

  if not m.api.OctoMail_Log[ "Settings" ].migrated and m.api.TurtleMail_Log then
    for _, log_type in ipairs( { "Sent", "Received" } ) do
      local old = m.api.TurtleMail_Log[ log_type ]
      if old and table.getn( old ) > 0 and table.getn( m.api.OctoMail_Log[ log_type ] ) == 0 then
        for _, entry in ipairs( old ) do
          table.insert( m.api.OctoMail_Log[ log_type ], entry )
        end
        while table.getn( m.api.OctoMail_Log[ log_type ] ) > m.LOG_MAX_ENTRIES do
          table.remove( m.api.OctoMail_Log[ log_type ], 1 )
        end
      end
    end
    table.insert( moved, "mail log" )
  end

  if m.api.TurtleMail_To and not m.api.OctoMail_To then
    m.api.OctoMail_To = m.api.TurtleMail_To
  end
  if m.api.TurtleMail_Point and not m.api.OctoMail_Point then
    m.api.OctoMail_Point = m.api.TurtleMail_Point
  end

  m.api.OctoMail_Log[ "Settings" ].migrated = true
  if table.getn( moved ) > 0 then
    m.info( "Imported your " .. table.concat( moved, " and " ) .. " from TurtleMail. You can remove TurtleMail now." )
  end
end

function OctoMail.ADDON_LOADED()
  if arg1 ~= "OctoMail" then return end

  local version = m.api.GetAddOnMetadata( "OctoMail", "Version" )
  m.info( string.format( "Loaded (|cffeda55fv%s|r).", version ) )

  m.migrate_from_turtlemail()

  if not m.api.OctoMail_Log[ "Settings" ].first_run then
    m.api.OctoMail_Log[ "Settings" ].first_run = version
    m.info( "Enable logging with |cffabd473/om log|r" )
  end
...
function OctoMail.PLAYER_LOGIN()
  ...
  local key = m.api.GetCVar( "realmName" ) .. "|" .. m.api.UnitFactionGroup( "player" )
  m.api.OctoMail_AutoCompleteNames[ key ] = m.api.OctoMail_AutoCompleteNames[ key ] or {}
  for char, last_seen in m.api.OctoMail_AutoCompleteNames[ key ] do
    -- Timestamps are time(), not GetTime(). GetTime() counts seconds since the
    -- client started and resets to ~0 every login, so a value written in an
    -- earlier session is almost always LARGER than the current one and this
    -- subtraction came out negative. The check could then only pass during a
    -- single unbroken 30 day session -- i.e. never -- so nothing was ever
    -- pruned and this list grew without bound.
    if last_seen < m.LEGACY_TIMESTAMP_CUTOFF then
      -- written by a build that stored GetTime(): a session counter, not a
      -- date, and there is no way to recover when it was really seen. Stamp it
      -- now so it ages out normally from here instead of never.
      m.api.OctoMail_AutoCompleteNames[ key ][ char ] = m.api.time()
    elseif m.api.time() - last_seen > m.AUTOCOMPLETE_MAX_AGE then
      m.api.OctoMail_AutoCompleteNames[ key ][ char ] = nil
    end
  end
  ...
  SLASH_OCTOMAIL1 = "/octomail"
  SLASH_OCTOMAIL2 = "/om"
  m.api.SlashCmdList[ "OCTOMAIL" ] = m.slash_command
end

function OctoMail.MAIL_SEND_SUCCESS()
...
function OctoMail.MAIL_INBOX_UPDATE()
  if m.inbox_opening then
    m.inbox_update = true
  end
  ...
  for i = 1, 7 do
    ...
    if index <= m.api.GetInboxNumItems() then
      ...
      if INBOX_AUCTIONHOUSES[ sender ] then
        m.api[ "OctoMailAuctionIcon" .. i ]:Show()
      else
        m.api[ "OctoMailAuctionIcon" .. i ]:Hide()
      end
      if was_returned then
        m.api[ "OctoMailReturnedArrow" .. i ]:Show()
      else
        m.api[ "OctoMailReturnedArrow" .. i ]:Hide()
      end
    end
  end
end
...
---@param name string
function OctoMail.add_auto_complete_name( name )
  local key = m.api.GetCVar( "realmName" ) .. "|" .. m.api.UnitFactionGroup( "player" )
  -- absolute unix time so the 30 day expiry survives a logout, see the prune
  -- loop in PLAYER_LOGIN
  m.api.OctoMail_AutoCompleteNames[ key ][ name ] = m.api.time()
end

function OctoMail.inbox_load()
  m.api.InboxFrame:EnableMouse( false )
  local btn = m.api.CreateFrame( "Button", "OctoMailOpenMailButton", m.api.InboxFrame, "UIPanelButtonTemplate" )
  ...
  for i = 1, 7 do
    m.api[ "OctoMailAuctionIcon" .. i .. "Texture" ]:SetVertexColor( m.api.NORMAL_FONT_COLOR.r, m.api.NORMAL_FONT_COLOR.g, m.api.NORMAL_FONT_COLOR.b )
    m.api[ "OctoMailReturnedArrow" .. i .. "Texture" ]:SetVertexColor( m.api.NORMAL_FONT_COLOR.r, m.api.NORMAL_FONT_COLOR.g, m.api.NORMAL_FONT_COLOR.b )
  end
end

function OctoMail.inbox_open_all()
...
function OctoMail.inbox_abort()
  m.inbox_opening = false
  m.inbox_update_lock()
  m.inbox_update = false
end

function OctoMail.set_cod_text()
...
---@param copper number
function OctoMail.format_money( copper )
...
---@param money number
function OctoMail.update_money( money )
...
function OctoMail.on_drag_stop()
  this:StopMovingOrSizing()
  local point, _, _, x, y = m.api.MailFrame:GetPoint()
  m.api.OctoMail_Point = { point = point, x = x, y = y }
end

---@param i number
---@param manual boolean?
function OctoMail.inbox_open( i, manual )
  m.debug( "inbox_open" )
...
function OctoMail.inbox_update_lock()
  for i = 1, 7 do
...
function OctoMail.hook.GetInboxHeaderInfo( ... )
...
function OctoMail.hook.OpenMail_Reply( ... )
  m.api.OctoMail_To = nil
  return m.orig.OpenMail_Reply( unpack( arg ) )
end

function OctoMail.hook.InboxFrame_Update()
...
---@param i number
function OctoMail.hook.InboxFrame_OnClick( i )
...
function OctoMail.hook.InboxFrameItem_OnEnter()
...
function OctoMail.hook.SendMailFrame_Update()
  local gap
...
function OctoMail.hook.SendMailRadioButton_OnClick( index )
  if (index == 1) then
    m.api.SendMailSendMoneyButton:SetChecked( 1 );
    m.api.SendMailCODButton:SetChecked( nil );
    m.api.SendMailMoneyText:SetText( m.api.AMOUNT_TO_SEND );
...
function OctoMail.hook.SplitContainerItem( bag, slot, amount )
  if m.sendmail_attached( bag, slot ) then return end
  return m.orig.SplitContainerItem( bag, slot, amount )
end

function OctoMail.hook.UseContainerItem( bag, slot, onself )
  if m.sendmail_attached( bag, slot ) then return end
...
function OctoMail.hook.SendMailFrame_CanSend()
...
function OctoMail.hook.MailFrameTab_OnClick( tab )
  if not tab then
    tab = this:GetID()
  end
  ...
  if tab == 3 then
    ...
    m.api.OctoMailLogFrame:Hide()
  end

  m.orig.MailFrameTab_OnClick( tab )
end

function OctoMail.hook.OpenMailFrame_OnHide()
...
function OctoMail.send_mail_button_onclick()
  m.api.MailAutoCompleteBox:Hide()

  m.api.OctoMail_To = m.api.SendMailNameEditBox:GetText()
  m.api.SendMailNameEditBox:HighlightText()

  m.sendmail_state = {
    to = m.api.OctoMail_To,
...
function OctoMail.sendmail_load()
  ...
  function m.api.SendMailNameEditBox:SetText( ... )
    if not m.api.OctoMail_To then
      return self:_SetText( unpack( arg ) )
    end
  end

  m.api.SendMailNameEditBox:SetScript( "OnShow", function()
    if m.api.OctoMail_To then
      m.api.this:_SetText( m.api.OctoMail_To )
    end
  end )
...
--@param bag number
--@param slot number
function OctoMail.sendmail_attached( bag, slot )
...
function OctoMail.attachment_button_on_click()
...
function OctoMail.sendmail_remove_attachment( item )
...
---@param item table
---@param slot number?
function OctoMail.sendmail_set_attachment( item, slot )
...
---@param item table
function OctoMail.sendmail_pickup_mailable( item )
  m.api.ClearCursor()
  m.orig.ClickSendMailItemButton()
  m.api.ClearCursor()
  m.orig.PickupContainerItem( unpack( item ) )
...
function OctoMail.sendmail_num_attachments()
  local x = 0
  for i = 1, ATTACHMENTS_MAX do
...
function OctoMail.sendmail_attachments()
  local t = {}
  for i = 1, ATTACHMENTS_MAX do
    local btn = m.api[ "MailAttachment" .. i ]
    if btn.item then
...
function OctoMail.sendmail_clear()
  local anyItem
  for i = 1, ATTACHMENTS_MAX do
...
function OctoMail.sendmail_send()
  ...
  if item then
    ...
    if not m.api.GetSendMailItem() then
      m.api.DEFAULT_CHAT_FRAME:AddMessage( "|cffabd473OctoMail|r: " .. m.api.ERROR_CAPS, 1, 0, 0 )
      return
    end
  end
...
do
  ...
  function OctoMail.previous_match()
    if index then
  ...
  function OctoMail.select_match( i )
    index = i
    complete()
    m.api.MailAutoCompleteBox:Hide()
  ...
  function GetSuggestions()
    ...
    for name, time in m.api.OctoMail_AutoCompleteNames[ m.api.GetCVar "realmName" .. "|" .. m.api.UnitFactionGroup "player" ] do
...
function OctoMail.log.load()
  m.api.OctoMailLogTitleText:SetText( L[ "Log" ] )
  ...
  for i = 1, 10 do
    m.api[ "OctoMailLogItem" .. i .. "Background" ]:SetVertexColor( .5, .5, .5, 0.6 )
    m.api[ "OctoMailLogItem" .. i .. "TimeStamp" ]:SetTextColor( 1, 1, 1, 1 )
    m.api[ "OctoMailLogItem" .. i .. "TimeStamp" ]:SetJustifyH( "LEFT" )
    m.api[ "OctoMailLogItem" .. i .. "TimeStamp" ]:SetFont( font_file, font_size )
    m.api[ "OctoMailLogItem" .. i .. "Participant" ]:SetTextColor( 1, 1, 1, 1 )
    m.api[ "OctoMailLogItem" .. i .. "Participant" ]:SetJustifyH( "RIGHT" )
    m.api[ "OctoMailLogItem" .. i .. "Participant" ]:SetFont( font_file, font_size )
    m.api[ "OctoMailLogItem" .. i .. "Subject" ]:SetJustifyH( "LEFT" )
    m.api[ "OctoMailLogItem" .. i .. "Subject" ]:SetFont( font_file, font_size )
    m.api[ "OctoMailLogItem" .. i .. "Money" ]:SetTextColor( 1, 1, 1, 1 )
    m.api[ "OctoMailLogItem" .. i .. "Money" ]:SetJustifyH( "LEFT" )
    m.api[ "OctoMailLogItem" .. i .. "Money" ]:SetFont( font_file, font_size )
    m.api[ "OctoMailLogItem" .. i .. "Status" ]:SetVertexColor( m.api.NORMAL_FONT_COLOR.r, m.api.NORMAL_FONT_COLOR.g, m.api.NORMAL_FONT_COLOR.b )
    if i > 1 then
      m.api[ "OctoMailLogItem" .. i ]:SetPoint( "TOPLEFT", m.api[ "OctoMailLogItem" .. i - 1 ], "BOTTOMLEFT", 0, -1 )
    end
  end
  m.api.OctoMailLogItem10Background:Hide()

  m.api.OctoMailLogStatusText:SetTextColor( 1, 1, 1, 1 )
  m.api.OctoMailLogStatusText:SetFont( "Fonts\\FRIZQT__.TTF", 10 )
  m.api.OctoMailLogScrollFrameScrollBar:SetValueStep( 1 )
  m.api.OctoMailLogScrollFrameScrollBar:SetScript( "OnValueChanged", m.log.on_scroll_value_changed )

  m.api.OctoMailLogScrollFrame:SetScript( "OnMouseWheel", function()
    m.log.scroll( arg1 * 10 )
  end )
  m.api.OctoMailLogScrollFrameScrollBarScrollUpButton:SetScript( "OnClick", function()
    m.api.PlaySound( "UChatScrollButton" );
    m.log.scroll( 10 )
  end )
  m.api.OctoMailLogScrollFrameScrollBarScrollDownButton:SetScript( "OnClick", function()
    m.api.PlaySound( "UChatScrollButton" );
    m.log.scroll( -10 )
  end )

  m.api.OctoMailLogFiltersButton:SetText( L[ "Filters" ] )
  m.api.OctoMailLogFiltersButton:GetFontString():SetPoint( "LEFT", m.api.OctoMailLogFiltersButton, "LEFT", 10, 0 )

  m.api.OctoMailLogFiltersButton:SetScript( "OnMouseDown", function()
    m.api.OctoMailLogFiltersButtonArrow:SetPoint( "RIGHT", m.pfui_skin_enabled and -4 or -8, -3 )
  end )
  m.api.OctoMailLogFiltersButton:SetScript( "OnMouseUp", function()
    m.api.OctoMailLogFiltersButtonArrow:SetPoint( "RIGHT", m.pfui_skin_enabled and -4 or -8, -1 )
  end )

  m.api.OctoMailLogStartTimeText:SetTextColor( 1, 1, 1, 1 )
  m.api.OctoMailLogStartTimeButton:SetScale( 0.9 )
  m.api.OctoMailLogEndTimeText:SetTextColor( 1, 1, 1, 1 )
  m.api.OctoMailLogEndTimeButton:SetScale( 0.9 )

  m.api.OctoMailLogStartTime:SetScript( "OnClick", function()
    m.api.OctoMailLogStartTimeText:SetText( "" )
    m[ m.current_log_type .. "_start_time" ] = nil
    m.log.populate( m.current_log_type )
  end )
  m.api.OctoMailLogEndTime:SetScript( "OnClick", function()
    m.api.OctoMailLogEndTimeText:SetText( "" )
    m[ m.current_log_type .. "_end_time" ] = nil
    m.log.populate( m.current_log_type )
  end )

  m.api.OctoMailLogPlayersDropDown:SetScale( 0.9 )
  m.api.UIDropDownMenu_SetText( "All players", m.api.OctoMailLogPlayersDropDown )

  m.dropdown_filters = m.api.CreateFrame( "Frame", "OctoMailDropDownFilters" )
  m.dropdown_filters.displayMode = "MENU"
  m.dropdown_filters.info = {}
end

function OctoMail.log.players_dropdown_on_load()
  m.api.UIDropDownMenu_Initialize( m.api.OctoMailLogPlayersDropDown, function()
    local info = {}
    info.notCheckable = 1
    ...
    local players = {}
    for _, v in ipairs( m.api.OctoMail_Log[ m.current_log_type ] ) do
      if v then
...
function OctoMail.log.select_player( player, is_all )
  m.api.UIDropDownMenu_SetText( player, m.api.OctoMailLogPlayersDropDown )
  if is_all then
    m.filter_player = nil
  else
...
function OctoMail.log.filter_dropdown()
  if m.dropdown_filters.initialize ~= OctoMail.log.filters_menu then
    m.api.CloseDropDownMenus()
    m.dropdown_filters.initialize = OctoMail.log.filters_menu
  end
  m.api.ToggleDropDownMenu( 1, nil, m.dropdown_filters, this:GetName(), 0, 0 )
end

function OctoMail.log.filters_menu( level )
  local filters = m.api.OctoMail_Log[ "Settings" ][ m.current_log_type .. "Filters" ] or {}
  local info = {}
...
function OctoMail.log.toggle_filter( filter, parent_filter )
  if not parent_filter then parent_filter = "" end
  local filter_value = m.api.OctoMail_Log[ "Settings" ][ m.current_log_type .. "Filters" ][ parent_filter .. filter ]

  m.api.OctoMail_Log[ "Settings" ][ m.current_log_type .. "Filters" ][ parent_filter .. filter ] = not filter_value
  m.log.populate( m.current_log_type )
end

function OctoMail.log.show_calendar()
  if m.calendar.is_visible() then
    m.calendar.hide()
  else
    local text = string.gsub( this:GetName(), "Button", "Text" )
    m.calendar.show( m.api.OctoMail_Log[ m.current_log_type ], time(), this, function( selected_date )
...
function OctoMail.log.scroll( step )
  local scroll_bar = m.api.OctoMailLogScrollFrameScrollBar
...
function OctoMail.log.on_scroll_value_changed()
  local function round( num )
    return num + (2 ^ 52 + 2 ^ 51) - (2 ^ 52 + 2 ^ 51)
  end

  local scrollBar = m.api.OctoMailLogScrollFrameScrollBar
  local scrollUp = m.api.OctoMailLogScrollFrameScrollBarScrollUpButton
  local scrollDown = m.api.OctoMailLogScrollFrameScrollBarScrollDownButton
...
---@param state table
function OctoMail.log.add( log_type, state )
  ...
  local log = m.api.OctoMail_Log[ log_type ]
  table.insert( log, data )

  -- Cap the log. Entries are appended oldest-first and nothing else removed
  -- them, so this table grew for the life of the character -- every mail ever
  -- sent or received, written to a SavedVariable and parsed back in full at
  -- each login. Trim from the front so the newest are the ones kept.
  while table.getn( log ) > m.LOG_MAX_ENTRIES do
    table.remove( log, 1 )
  end
end
...
---@param log_type LogType
---@param index number?
function OctoMail.log.populate( log_type, index )
  m.current_log_type = log_type
  local filters = m.api.OctoMail_Log[ "Settings" ][ log_type .. "Filters" ] or {}
  ...
  local log = m.filter( m.api.OctoMail_Log[ log_type ], function( item )
    local ret =
  ...
  m.api.OctoMailLogStartTimeText:SetText( start_time and date( L[ "date_format" ], start_time ) or "" )
  m.api.OctoMailLogEndTimeText:SetText( end_time and date( L[ "date_format" ], end_time ) or "" )

  if not log then return end
  local log_count = getn( log )

  m.api.OctoMailLogScrollFrameScrollBar:SetMinMaxValues( 0, math.max( 0, log_count - 10 ) )

  if not index then
    m.api.OctoMailLogScrollFrameScrollBar:SetValue( log_count - 10 )
    m.api.OctoMailLogScrollFrameScrollBar:SetScript( "OnUpdate", function()
      m.api.OctoMailLogScrollFrameScrollBar:SetValue( log_count - 10 )
      m.api.OctoMailLogScrollFrameScrollBar:SetScript( "OnUpdate", nil )
    end )

    index = math.max( 0, log_count - 10 )
  end

  m.api.OctoMailLogTitleText:SetText( string.format( "%s %s", L[ log_type ], L[ "Log" ] ) )
  m.api.OctoMailLogStatusText:SetText( string.format( "Showing %d-%d of %d", (index == 0 and log_count == 0) and index or index + 1,
  ...
  for i = 1, 10 do
    if log[ index + i ] then
      ...
      m.api[ "OctoMailLogItem" .. i .. "IconTexture" ]:SetTexture( entry.icon or "Interface/Icons/INV_Misc_Note_01" )
      m.api[ "OctoMailLogItem" .. i .. "Icon" ].item = entry.item
      m.api[ "OctoMailLogItem" .. i .. "TimeStamp" ]:SetText( date( L[ "date_format" ] .. " " .. L[ "time_format" ], entry.timestamp ) )
      m.api[ "OctoMailLogItem" .. i .. "Subject" ]:SetText( entry.subject )

      m.api[ "OctoMailLogItem" .. i .. "Status" ]:SetTexture( "" )
      if entry.ah then
        m.api[ "OctoMailLogItem" .. i .. "Participant" ]:SetText( "" )
        m.api[ "OctoMailLogItem" .. i .. "Status" ]:SetTexture( "Interface\\Addons\\OctoMail\\OctoMail-AH.blp" )
        m.api[ "OctoMailLogItem" .. i .. "Status" ]:SetPoint( "TOPRIGHT", 4, -10 )
      else
        m.api[ "OctoMailLogItem" .. i .. "Participant" ]:SetText( entry.participant )
      end
      if entry.returned then
        m.api[ "OctoMailLogItem" .. i .. "Status" ]:SetTexture( "Interface\\Addons\\OctoMail\\OctoMail-RetArrow.blp" )
        local w = m.api[ "OctoMailLogItem" .. i .. "Participant" ]:GetStringWidth()
        m.api[ "OctoMailLogItem" .. i .. "Status" ]:SetPoint( "TOPRIGHT", -w + 4, -9 )
      end

      if entry.money and entry.money > 0 then
        local cod = (entry.cod and entry.cod > 0) and "COD: " or " "
        m.api[ "OctoMailLogItem" .. i .. "Money" ]:SetText( cod .. m.format_money( entry.money ) )
      elseif entry.cod then
        m.api[ "OctoMailLogItem" .. i .. "Money" ]:SetText( "COD" .. (entry.cod > 1 and (": " .. m.format_money( entry.cod )) or "") )
      else
        m.api[ "OctoMailLogItem" .. i .. "Money" ]:SetText( "" )
      end

      m.api[ "OctoMailLogItem" .. i ]:Show();
    else
      m.api[ "OctoMailLogItem" .. i ]:Hide();
    end
  end
end

function OctoMail.pfui_skin()
  if m.api.IsAddOnLoaded( "pfUI" ) and m.api.pfUI and m.api.pfUI.api and m.api.pfUI.env and m.api.pfUI.env.C then
    m.api.pfUI:RegisterSkin( "OctoMail", "vanilla", function()
      -- Don't apply our skin when Mailbox skin is disabled because it doesn't make sense.
      if m.api.pfUI.env.C.disabled and (m.api.pfUI.env.C.disabled[ "skin_OctoMail" ] == "1" or m.api.pfUI.env.C.disabled[ "skin_Mailbox" ] == "1") then
        return
      end
      ...
      --Inbox
      m.api.pfUI.api.SkinButton( m.api.OctoMailOpenMailButton )
      ...
      m.api.OctoMailLogTitleText:ClearAllPoints()
      m.api.OctoMailLogTitleText:SetPoint( "TOP", m.api.MailFrame.backdrop, "TOP", 0, -10 )

      m.api.pfUI.api.CreateBackdrop( m.api.OctoMailLogScrollFrame )

      m.api.OctoMailLogScrollFrame:SetHeight( 307 )
      m.api.OctoMailLogScrollFrame:ClearAllPoints()
      m.api.OctoMailLogScrollFrame:SetPoint( "TOPLEFT", 23, -90 )
      m.api.OctoMailLogScrollFrame:SetWidth( 299 )
      m.api.OctoMailLogEntriesFrame:ClearAllPoints()
      m.api.OctoMailLogEntriesFrame:SetPoint( "TOPLEFT", 23, -90 )
      m.api.OctoMailLogEntriesFrame:SetWidth( 299 )

      m.api.pfUI.api.StripTextures( m.api.OctoMailLogItem10 )
      m.api.pfUI.api.SkinScrollbar( m.api.OctoMailLogScrollFrameScrollBar )
      m.api.OctoMailLogScrollFrameScrollBar:SetPoint( "TOPLEFT", m.api.OctoMailLogScrollFrame, "TOPRIGHT", 6, -14 )
      m.api.OctoMailLogScrollFrameScrollBar:SetPoint( "BOTTOMLEFT", m.api.OctoMailLogScrollFrame, "BOTTOMRIGHT", 6, 14 )

      m.api.pfUI.api.SkinButton( m.api.OctoMailLogSentButton )
      m.api.pfUI.api.SkinButton( m.api.OctoMailLogReceivedButton )
      m.api.OctoMailLogReceivedButton:ClearAllPoints()
      m.api.OctoMailLogReceivedButton:SetPoint( "RIGHT", m.api.OctoMailLogSentButton, "LEFT", -2 * bpad, 0 )

      m.api.pfUI.api.SkinButton( m.api.OctoMailLogFiltersButton )
      m.api.OctoMailLogFiltersButton:SetPoint( "TOPLEFT", 21, -58 )
      m.api.OctoMailLogFiltersButton:GetFontString():SetFont( m.api.pfUI.font_default, 12 )
      m.api.OctoMailLogFiltersButton:GetFontString():SetPoint( "TOPLEFT", 4, -4.5 )
      m.api.OctoMailLogFiltersButton:SetWidth( 50 )
      m.api.OctoMailLogFiltersButton:SetHeight( 20 )
      m.api.OctoMailLogFiltersButtonArrow:SetPoint( "RIGHT", -4, -1 )
      m.api.OctoMailLogFiltersButtonArrow:SetWidth( 8.5 )
      m.api.OctoMailLogFiltersButtonArrow:SetHeight( 8.5 )
      m.api.OctoMailLogFiltersButtonArrow:SetTexture( m.api.pfUI.media[ "img:down" ] )

      m.api.pfUI.api.StripTextures( m.api.OctoMailLogStartTime )
      m.api.pfUI.api.SkinButton( m.api.OctoMailLogStartTime )
      m.api.pfUI.api.SkinArrowButton( m.api.OctoMailLogStartTimeButton, "down", 16 )
      m.api.OctoMailLogStartTime:SetPoint( "TOPLEFT", 77, -58 )
      m.api.OctoMailLogStartTimeText:SetPoint( "LEFT", 5, 0 )
      m.api.OctoMailLogStartTimeButton:SetPoint( "LEFT", m.api.OctoMailLogStartTime, "RIGHT", -19, 0 )
      m.api.OctoMailLogStartTimeTitle:SetPoint( "TOPLEFT", 1, 13 )
      m.api.OctoMailLogStartTimeTitle:SetFont( m.api.pfUI.font_default, 10 )
      m.api.OctoMailLogStartTimeTitle:SetText( L[ "Period start" ] )

      m.api.pfUI.api.StripTextures( m.api.OctoMailLogEndTime )
      m.api.pfUI.api.SkinButton( m.api.OctoMailLogEndTime )
      m.api.pfUI.api.SkinArrowButton( m.api.OctoMailLogEndTimeButton, "down", 16 )
      m.api.OctoMailLogEndTime:SetPoint( "TOPLEFT", 163, -58 )
      m.api.OctoMailLogEndTimeText:SetPoint( "LEFT", 5, 0 )
      m.api.OctoMailLogEndTimeButton:SetPoint( "LEFT", m.api.OctoMailLogEndTime, "RIGHT", -19, 0 )
      m.api.OctoMailLogEndTimeTitle:SetPoint( "TOPLEFT", 1, 13 )
      m.api.OctoMailLogEndTimeTitle:SetFont( m.api.pfUI.font_default, 10 )
      m.api.OctoMailLogEndTimeTitle:SetText( L[ "Period end" ] )

      m.api.pfUI.api.SkinDropDown( m.api.OctoMailLogPlayersDropDown, nil, nil, nil, true )
      m.api.UIDropDownMenu_SetWidth( 90, m.api.OctoMailLogPlayersDropDown )
      m.api.OctoMailLogPlayersDropDown:SetScale( 1 )
      m.api.OctoMailLogPlayersDropDown:SetPoint( "TOPLEFT", 224, -54 )
      m.api.OctoMailLogPlayersDropDown.backdrop:SetPoint( "TOPLEFT", 26, -4 )
      m.api.OctoMailLogPlayersDropDown.backdrop:SetPoint( "BOTTOMRIGHT", -20, 8 )
      m.api.OctoMailLogPlayersDropDown.button.backdrop:SetWidth( 16 )
      m.api.OctoMailLogPlayersDropDown.button.backdrop:SetHeight( 16 )

      local label = m.api.OctoMailLogFrame:CreateFontString( nil, "BACKGROUND", "GameFontNormal" )
...
function OctoMail.filter( t, f, extract_field )
...
function OctoMail.info( message )
  m.api.DEFAULT_CHAT_FRAME:AddMessage( string.format( "|cffabd473OctoMail|r: %s", message ) )
end

function OctoMail.dump( o )
...
function OctoMail.debug( ... )
  if m.debug_enabled then
    ...
    for i = 1, getn( arg ) do
      ...
      if message then
        ...
        if type( message ) == 'table' then
          messages = messages .. OctoMail.dump( message )
        else
    ...
    m.api.DEFAULT_CHAT_FRAME:AddMessage( string.format( "|cffabd473OctoMail|r: %s", messages ) )
  end
end
